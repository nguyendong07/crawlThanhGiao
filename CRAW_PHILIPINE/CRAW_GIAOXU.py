...
workbook = xlsxwriter.Workbook("../DATA_PHILIPINE/GIAOXU.xlsx")
...
with open('../DATA_PHILIPINE/html.txt', encoding="utf8") as f:
    lines = f.read()
    ...
    for child in children:
        try:
            if child.find("a"):
                ...
                if len(church_tag) >= 2:
                    ...
                    for church in list_church:
                        ...
                        # link_church_deltail = "http://www.gcatholic.org/churches/PHILIPINE/78833.htm"
                        ...
                        if div_thumb:
                            ...
                            DOWNLOAD_IMAGE.download(Link_Thumb,COMMON_URL.LIST_IMAGES_GIAOXU.get("PHILIPINE"))
                        ...
                        for att_detail in list_att_p:
                            ...
                            if "Sector" in str(att_detail.findChildren("span", {"class": "label"})):
                                if att_detail.find("a"):
                                    for att in att_detail.findChildren("a"):
                                        Sector = Sector + "," + att.string
                                        Sector = remove_firstindex(Sector)
                            ...
                            if "Address" in str(att_detail.findChildren("span", {"class": "label"})):
                                if att_detail.contents[1]:
                                    Address = att_detail.contents[1] + "," + att_detail.find("span",{"class": "zregion"}).string
                                else:
                        ...
                        print(Sector, Region,Deanery, Name_Church,Jurisdiction, Served_By, Type, BasilicaDecree, Link_Thumb, Rite, Lists, History, Patron,
                              Location,
            ...
            #     worksheet.write(index + 1, 16, entry["Name_Church"])
...
worksheet.write(0, 16, "Name")
worksheet.write(0, 17, "Region")
worksheet.write(0, 18, "Deanery")
worksheet.write(0, 19, "Sector")
for index, entry in enumerate(arr):
    worksheet.write(index + 1, 0, entry["Jurisdiction"])
    worksheet.write(index + 1, 1, entry["Served_By"])
    worksheet.write(index + 1, 2, entry["Type"])
    ...
    worksheet.write(index + 1, 17, entry["Region"])
    worksheet.write(index + 1, 18, entry["Deanery"])
    worksheet.write(index + 1, 19, entry["Sector"])
workbook.close()
